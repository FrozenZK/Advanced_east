...
def preprocess():
    ...
    # 测试数据预处理
    origin_test_dir = os.path.join(data_dir, cfg.test_origin_image_dir_name)
    origin_txt_test_dir = os.path.join(data_dir, cfg.test_origin_txt_dir_name)
    ...
    for o_img_fname, _ in zip(o_img_list, tqdm(range(len(o_img_list)))):
        with Image.open(os.path.join(origin_test_dir, o_img_fname)) as im:
            ...
            # draw on the img
            draw = ImageDraw.Draw(show_gt_im)
            with open(os.path.join(origin_txt_test_dir,
